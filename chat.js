...
wss.on('connection', function connection(ws) {
	...
	ws.on('message', function (msg) {
		...
		if (data.connect == 'first') { //première connection
			...
			if (ws.pseudo == "SERVEUR") //on reserve certains noms
		...
		else if (data.type != 'getStats' && data.type != 'pseudo?' && data.type != 'ping' && data.type != 'connect' && data.type != 'askPseudo') { //certaines commandes
			if (salle[ws.room].msg.length > 1000)
				salle[ws.room].msg.shift();
			salle[ws.room].msg.push(data);
		}
		else if (data.type == 'pseudo?') { //demande si le pseudo existe déjà
			ws.room = data.room;
			if (salle[ws.room] !== undefined)
		...
		if (data.type == "connect") { //unde personne vient de se connecter, on garde son nom et sa salle en memoire
			let id = generateId();
			let msg = {
				type: 'connect',
				id: id
		...
		else if (data.type == "askPseudo") { //on recup un pseudo par un id
			let msg = {
				type: 'askPseudo',
		...
		data.pseudo = ws.pseudo; //sert presque a rien vu quon peut juste utiliser ws. tout le temps




		if (data.type == "chatMsg" || data.connect == 'first' || data.type == 'disconnect' || data.type == 'ping') //le type de requete a broadcast
		...
		if (data.type == "ping") { //un ping, verifie les gens connectés
			var ping = {
				type: 'pong',
				msg: 'nbClients :) : ',
		...
		if (data.type == 'getStats') { //on recup des stats
